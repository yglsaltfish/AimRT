...
def my_log_check(context: Dict[str, Any]) -> CallbackResult:
    ...
    # 自定义检查逻辑
    if "Benchmark plan 2 completed" not in stdout:
...
class TestRPCExamples:
    @pytest.mark.aimrt
    def test_ros2_reliable_yaml_config(self, aimrt_test_runner: AimRTTestRunner):
        yaml_config_path = Path(__file__).parent / "test_ros2.yaml"
    ...
    @pytest.mark.aimrt
    def test_ros2_besteffort_yaml_config(self, aimrt_test_runner: AimRTTestRunner):
        yaml_config_path = Path(__file__).parent / "test_ros2.yaml"
        ...
        print("✅ YAML-based channel test completed successfully")
