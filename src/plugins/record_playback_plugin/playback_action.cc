...
void PlaybackAction::Initialize(YAML::Node options_node) {
  ...
  for (auto& item : metadata_.files) {
    std::string db_file_path = std::filesystem::path(options_.bag_path) / item.path;

    AIMRT_CHECK_ERROR_THROW(
        std::filesystem::exists(db_file_path) && std::filesystem::is_regular_file(db_file_path),
        "Can not find bag file '{}' in bag path '{}'.", db_file_path, options_.bag_path);
  }
...
bool PlaybackAction::OpenNewDb() {
  ...
  std::string db_file_path = std::filesystem::path(options_.bag_path) / file.path;
  ++cur_db_file_index_;
