...
class ChannelBackendManagerTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    // 测试用发布类型包装器，用于初始化
    publish_type_wrapper_test.msg_type = "publish_msg_type_test";
    publish_type_wrapper_test.pkg_path = "publish_pkg_path_test";
    publish_type_wrapper_test.module_name = "publish_module_name_test";
    publish_type_wrapper_test.topic_name = "publish_topic_name_test";

    // 测试用订阅包装器，用于初始化
    subscribe_wrapper_test.msg_type = "subscribe_msg_type_test";
    subscribe_wrapper_test.pkg_path = "subscribe_pkg_path_test";
    subscribe_wrapper_test.module_name = "subscribe_module_name_test";
    subscribe_wrapper_test.topic_name = "subscribe_topic_name_test";

    // 测试用发布包装器，用于程序运行
    publish_wrapper_test.msg_type = "publish_msg_type_test";
    publish_wrapper_test.pkg_path = "publish_pkg_path_test";
    publish_wrapper_test.module_name = "publish_module_name_test";
    publish_wrapper_test.topic_name = "publish_topic_name_test";
    publish_wrapper_test.ctx_ref = aimrt::channel::ContextRef();
    ...
    channel_backend_manager_.RegisterChannelBackend(mock_backend_test_ptr_.get());
    channel_backend_manager_.SetPubTopicsBackendsRules(pub_topics_backends_rules_test_);
    channel_backend_manager_.SetSubTopicsBackendsRules(sub_topics_backends_rules_test_);
  ...
  std::unique_ptr<MockChannelBackend> mock_backend_test_ptr_ = std::make_unique<MockChannelBackend>();
  ...
  PublishTypeWrapper publish_type_wrapper_test;
  SubscribeWrapper subscribe_wrapper_test;
  PublishWrapper publish_wrapper_test;
};
...
TEST_F(ChannelBackendManagerTest, RegisterPublishType) {
  ...
  EXPECT_TRUE(channel_backend_manager_.RegisterPublishType(std::move(publish_type_wrapper_test)));
...
TEST_F(ChannelBackendManagerTest, Subscribe) {
  ...
  EXPECT_TRUE(channel_backend_manager_.Subscribe(std::move(subscribe_wrapper_test)));
...
TEST_F(ChannelBackendManagerTest, Publish) {
  ...
  EXPECT_TRUE(channel_backend_manager_.RegisterPublishType(std::move(publish_type_wrapper_test)));
  channel_backend_manager_.Publish(publish_wrapper_test);
