    ...
    void Start() {
      ...
      boost::asio::co_spawn(
          ...
          [this, self]() -> boost::asio::awaitable<void> {
            try {
              remote_addr_ = util::SSToString(session_options_ptr_->svr_ep);

              AIMRT_TRACE("Tcp cli session start create a new connect to {}.",
                          RemoteAddr());
                  ...
                  [this, self]() -> boost::asio::awaitable<void> {
                    try {
                      while (state_.load() == SessionState::Start) {
                        ...
                        if (heartbeat_flag) {
                          // 心跳包仅用来保活，不传输业务/管理信息
                          static constexpr char heartbeat_pkg[HEAD_SIZE] = {
                              HEAD_BYTE_1, HEAD_BYTE_2, 0, 0, 0, 0};
                          static const boost::asio::const_buffer heartbeat_buf(
                              heartbeat_pkg, HEAD_SIZE);
      ...
      boost::asio::dispatch(session_socket_strand_, [this, self]() {
        ...
        while (stop_step) {
          try {
            ...
            AIMRT_TRACE(
                "Tcp cli session stop get exception at step {}, exception info: {}",
                stop_step, e.what());
            ++stop_step;
          }
        }
...
class AsioTcpClientPool
 ...
 public:
  ...
  boost::asio::awaitable<std::shared_ptr<AsioTcpClient>> GetClient(
